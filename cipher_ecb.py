...
#Gives you the choice of what function you want
def main():
	global debug
	...
	while (notquit):
		if debug:
			thirdOption = "To disable debug mode"
		else:
			thirdOption = "To enable debug mode"
		print
...
def bitShift(block, left, rotation):
	...
	if debug:
		...
		if left:
			...
			direction = "left"
		print "Bit Shift:",ba.to01(),"(",rotation,"to the",direction,")"
	return output
...
def encrypt(filename):
	...
	cipherlist = []
	if debug:
		print "On debug mode, every block is shown. To move to the next one simple press Enter\n"
